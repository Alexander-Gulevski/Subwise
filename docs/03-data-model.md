...
model Subscription {
  ...
  /// Якорь расписания: исходная дата, задающая день месяца.
  /// НЕ меняется при наступлении списаний — см. раздел 5
...
### Зачем отдельное поле `billingAnchorAt`

`nextBillingAt` меняется после каждого списания, поэтому опираться на него нельзя: подписка от 31 января, пересчитанная от 28 февраля, «сползёт» на 28 число и останется там навсегда. Пользователь потеряет три дня в месяц, а через год расписание перестанет иметь отношение к реальным списаниям.

`billingAnchorAt` фиксируется при создании и меняется **только** когда пользователь явно правит дату. Побочный выигрыш: после снятия с паузы день месяца тоже не съезжает — возобновление считается от якоря, а не от сегодняшней даты.

У триала якорем становится дата окончания триала: именно с неё начинаются платные списания.
